...
An example of a system made for a game in which four players play in two teams can be seen in figure \ref{fig:four-player-model} and a factor graph expansion can be found in Appendix \ref{factor-graph}.
...
The designed model includes several hyper-parameters which have to be adjusted for each game:
\begin{enumerate}
...
\end{enumerate}
They are ranked in order of importance, adjusting the first one gives the most accuracy increase, but also increases resources required the most.
