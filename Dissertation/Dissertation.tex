...
\item[Year] 2017
\item[Word Count] \footnote{Calculated using \mintinline{bash}{pdftotext Disseration.tex -f 7 -l 38 - | wc -w} and manually excluding appendices and bibliography} %TODO
...
If the training period is not long enough the algorithm will be under-fit and won't perform at its optimum.
Similarly, if the training period is too long, the algorithm might over-fit and again won't perform at its optimum;
...
\item[Using recurrent neural network for skill update]
The skill update model treats player performances as observations, therefore using RNN with previous $n$ games as input might produce better results.
