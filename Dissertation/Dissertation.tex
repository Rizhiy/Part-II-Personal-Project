...
\caption{Fitted gaussian and gamma distributions, since gamma distribution has lower $\chi^2$ value, it is a better fit.}
...
\node[latent] (t) [below=of p] {$t_j$};
\draw[edge] (p) to (t);

\node[obs] (x) [right=of p] {$x_i^j$};
\draw[edge] (p) to (x);
\draw[edge] (t) to (x);

\node[obs] (y) [right=3cm of t] {$y$};
\draw[edge] (t) to (y);

\plate {i} {(p)(x)} {$i\subset j$};
\plate[inner sep=0.3cm] {j} {(p)(x)(t)} {$j\subset T$};
\plate[inner sep=0.5cm] {m} {(p)(x)(t)(y)} {$m\subset M$};

\node[latent] (s) [left=of p] {$s_n$};
\draw[edge] (s) to (p);
\plate {n} {(s)} {$n\subset N$};
\end{tikzpicture}
\caption{Summed up view of the model.
There are $N$ players tracked by the system, participating in $M$ matches.
In each match there are $T$ teams.
$x_i$ are the player results and $y$ are the overall match results.
All nodes represent vectors.}
...
\end{itemize}
The model further assumes that player specific results depend on skills of the relevant player and skills of each teams, and overall results only depend on skills of each team.
...
Instead I decided to use the technique called \emph{coordinate descent}.
