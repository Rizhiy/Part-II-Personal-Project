...
\begin{center}
\Huge
\textbf{Predicting statistical results in competitive computer team games} \\[5mm]
...
\item[Title] Predicting statistical results in competitive computer team games.
\item[Examination] Computer Science Tripos -- Part II, June 2017
\item[Word Count] 9263\footnote{Calculated using \mintinline{bash}{pdftotext Disseration.tex -f 7 -l 38 - | wc -w} and manually excluding appendices and bibliography} %TODO
...
The system achieved $R^2$ value of 0.5 on one variable in the dataset and over 0.25 on another three.
\subsection*{Special Difficulties}
There was a mistake in the project proposal success criteria, which is explained in subsection \ref{requirements-error}.
...
Team skills are also latent variables which have to be estimated from game results.
...
Using neural networks for conditional probabilities, meant that Bayes' theorem could not be used to make a model for performance estimate, since integrating out a variable produced by a neural network requires integration over all possible weights in a neural network which is infeasible.
To solve that problem variational methods were used to approximate the reverse conditional probability using sampling.

\chapter{Evaluation}
This chapter discusses how the final system was evaluated.
It introduces \emph{train/validation/test} split and how it is used to evaluate a machine learning model.
...
\note{\emph{GPM} and \emph{XPM} depends mainly on player's playstyle and changes little based on other player in the game, therefore they were predicted well.
...
\begin{minted}[frame=lines,linenos,fontsize=\small]{json}
{  
   "result":{  
      "players":[  
         {  
...
This depends on complexity of the game and amount of variables which the system has to predict.
...
This hyper-parameter depends on how quickly the strategies used by players change.
If the same strategies are used for long periods of time, then much older matches can be considered then if strategies constantly change.
\end{description}
...
\end{enumerate}
Theses are standard machine learning parameters and adjusting them is a complicated task with many possible approaches.
